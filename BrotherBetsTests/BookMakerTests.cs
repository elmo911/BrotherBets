...
namespace BrotherBetsTests
{
    public class BookMakerTests
    {
        ...
        public void AddBet_BetNull_ArgumentNullEx()
        {
            ...
            Assert.Throws<ArgumentNullException>(() => bookie.AddBet(null, new Bettor(), new Brother(), new[] {""}));
        }
        [Fact]
        public void AddBet_CreatorNull_ArgumentNullEx()
        {
            var bookie = new BookMaker();
            Assert.Throws<ArgumentNullException>(() => bookie.AddBet(new Bet(), new Bettor(), null, new[] { "" }));
        }
        [Fact]
        public void AddBet_OutcomesNull_ArgumentNullEx()
        {
            var bookie = new BookMaker();
            Assert.Throws<ArgumentNullException>(() => bookie.AddBet(new Bet(), new Bettor(), new Brother(), null));
        }
        [Fact]
        public void AddBet_EmptyPredictions_ThrowsException()
        {
            ...
            Assert.Throws<Exception>(() => bookie.AddBet(new Bet(), new Bettor(), new Brother(), new[] {"", "", ""}));
        }
        [Fact]
        public void AddBet_OnePrediction_ThrowsException()
        {
            var betRepoMock = new Mock<IBetRepository>();
            var bookie = new BookMaker(betRepoMock.Object);
            Assert.Throws<Exception>(() => bookie.AddBet(new Bet(), new Bettor(), new Brother(), new[] { "Outcome1", "" }));
        }
        ...
        public void AddBet_ValidInput_AddsBet()
        {
            var betRepoMock = new Mock<IBetRepository>();
            var bookie = new BookMaker(betRepoMock.Object);
            bookie.AddBet(new Bet(), new Bettor(), new Brother(), new[] { "Outcome1", "Outcome2", "", "" });
            betRepoMock.Verify(m => m.Add(It.IsAny<Bet>(), It.IsAny<int>(), It.IsAny<int>()));
        }
